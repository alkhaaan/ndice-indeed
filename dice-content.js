...
async function collectJobsFromSearch(payload) {
  ...
  const startFromFirstPage = payload?.startFromFirstPage !== false;
  const maxPaginationPages = Math.max(
    1,
    Math.min(25, Number.isFinite(requestedPages) && requestedPages > 0 ? requestedPages : 3)
  );
  ...
  for (let pageIndex = 0; pageIndex < maxPaginationPages; pageIndex += 1) {
    ...
    for (const card of cards) {
      ...
      if (!item) {
        const dropReason = getCardDropReason(card);
        if (dropReason === "missing_anchor") {
          droppedNoAnchor += 1;
        } else if (dropReason === "missing_href") {
          droppedNoHref += 1;
        } else if (dropReason === "non_job_url") {
          droppedNonJobUrl += 1;
        }
        continue;
      }
  ...
  await log("info", "discover", "Collected jobs from search page.", {
    ...
    dropped: {
      missingAnchor: droppedNoAnchor,
      missingHref: droppedNoHref,
      nonJobUrl: droppedNonJobUrl
    },
    pagesVisited,
    pageAdvanceCount,
    stopReason,
...
async function ensureResultsStartPage() {
  const pageOneUrl = buildPageOneUrl(location.href);
  if (!pageOneUrl || pageOneUrl === location.href) {
    return;
  }

  const beforeUrl = location.href;
  const beforeFingerprint = getResultsFingerprint();
  location.assign(pageOneUrl);
  await waitForResultsPageChange(beforeUrl, beforeFingerprint, 10000);
}
...
function buildPageOneUrl(currentUrl) {
  try {
    const url = new URL(currentUrl);
    const currentPage = Number(url.searchParams.get("page") || "1");
  ...
  for (let step = 0; step < maxFormSteps; step += 1) {
    ...
    if (submitBtn) {
      ...
      if (isSubmissionConfirmationPage() || !isApplyModalOpen()) {
        return {
          status: "submitted",
          details: "Submitted from wizard final action."
        };
      }

      return {
        status: "manual_review_required",
        details: "Final submit was attempted but confirmation was not detected."
      };
    }
...
function collectJobCards() {
  ...
  // Always add explicit job-detail anchors as a fallback. Dice sometimes renders
  // non-job containers that match generic selectors and would otherwise suppress this path.
  const anchors = Array.from(document.querySelectorAll("a[href*='/job-detail/'], a[href*='jobId=']"));
...
function isLikelyJobCard(card) {
  if (!card) {
    return false;
  }

  const hasJobLink = Boolean(card.querySelector("a[href*='/job-detail/'], a[href*='jobId=']"));
  if (hasJobLink) {
    return true;
  }

  const id = normalizeText(card.getAttribute("id") || "");
  const dataJobId = normalizeText(card.getAttribute("data-jobid") || card.dataset?.jobid || card.dataset?.jobId || "");
  if (/^position-card-/i.test(id) || Boolean(dataJobId)) {
    return true;
  }
...
function getCardDropReason(card) {
  const anchor =
    card.querySelector("a[data-cy='card-title-link']") ||
    card.querySelector("a[href*='/job-detail/']") ||
    card.querySelector("a[href*='jobId=']") ||
  ...
  try {
    const url = new URL(hrefValue, location.origin).href;
    if (!/\/job-detail\/|[?&]jobId=/i.test(url)) {
      return "non_job_url";
    }
  } catch (error) {
    return "non_job_url";
  ...
  for (let step = 0; step < maxFormSteps; step += 1) {
    ...
    if (submitBtn) {
      ...
      if (isSubmissionConfirmationPage() || !isApplyModalOpen()) {
        return {
          status: "submitted",
          details: "Submitted from apply flow final action."
        };
      }
    ...
    if (!isApplyModalOpen()) {
      return {
        status: "manual_review_required",
        details: "Apply flow closed after navigation without confirmation."
      };
    }
